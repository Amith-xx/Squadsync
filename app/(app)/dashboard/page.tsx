...
import type { PlayerAttributes, LobbyListItem } from "@/types";
...
export default async function DashboardPage() {
  ...
  const overall = user?.attributes ? computeOverall(user.attributes as PlayerAttributes) : 60;
  ...
  return (
    <main className="mx-auto max-w-4xl px-4 py-8 sm:py-12">
      ...
      <div className="mb-8 animate-fade-in-up">
        ...
        <h1 className="mt-0.5 text-3xl font-black text-white">
      ...
      <div className="mb-8 grid grid-cols-3 gap-3">
        {[
          ...
          {
            label: "Karma Score",
            value: String(user?.karmaScore ?? 70),
            accent: false,
          },
          {
      ...
      <div className="mb-8 rounded-xl border border-navy-border bg-navy-dark px-5 py-4">
        ...
        <div className="mt-1 flex items-baseline gap-2">
          <span className="text-2xl font-black text-white">{user?.position ?? "—"}</span>
      ...
      {region ? (
        ...
        <div className="mb-8 rounded-2xl border border-yellow-400/20 bg-yellow-400/5 p-6">
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-xl bg-yellow-400/15 px-4 py-2 text-sm font-bold text-yellow-400 transition-colors hover:bg-yellow-400/25 border border-yellow-400/20"
          >
            Set Region
          </Link>
        </div>
      ...
      <Link
        href="/profile"
        className="group flex flex-col rounded-2xl border border-neon-green/20 bg-neon-green/5 p-6 transition-all hover:border-neon-green/40 hover:bg-neon-green/10 hover:shadow-[0_0_24px_rgba(0,230,115,0.15)]"
      >
        <span className="inline-block rounded-full bg-neon-green/15 px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-widest text-neon-green border border-neon-green/20">
          Ready
        </span>
